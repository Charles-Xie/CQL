...
class HICOEvaluator():
    def __init__(self, preds, gts, subject_category_id, rare_triplets, non_rare_triplets, correct_mat):
        ...
        for img_preds in preds:
            ...
            bboxes = [{'bbox': bbox, 'category_id': label, 'score': score} for bbox, label, score in zip(img_preds['boxes'], img_preds['labels'], img_preds['scores'])]
        ...
        self.gts = []
        for img_gts in gts:
            img_gts = {k: (v.to('cpu').numpy() if k not in ('id', 'file_name') else v) for k, v in img_gts.items()}
            self.gts.append({
    ...
    def get_predictions(self):
        preds = jsonify(deepcopy(self.preds))
        for img, img_gt in zip(preds, self.gts):
            img['file_name'] = img_gt['file_name']
            for det in img['predictions']:
                det['category_id'] = self._valid_obj_ids[det['category_id']]
            for hoi in img['hoi_prediction']:
                hoi['category_id'] = self._valid_verb_ids[hoi['category_id']]
        return preds
