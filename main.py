...
def get_args_parser():
    ...
    parser.add_argument('--verb_loss_type', type=str, default='focal', choices=('bce', 'focal', 'asl'),
    ...
    parser.add_argument('--image_verb_loss_coef', default=1, type=float)
...
def main(args):
    ...
    if args.eval:
        if args.hoi:
            test_stats, _ = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val, args.subject_category_id, device, args.output_dir)
            return
        else:
    ...
    for epoch in range(args.start_epoch, args.epochs):
        ...
        if args.hoi:
            test_stats, preds = evaluate_hoi(args.dataset_file, model, postprocessors, data_loader_val, args.subject_category_id, device, args.output_dir)
            coco_evaluator = None
        else:
        ...
        current_map = test_stats['mAP_all'] if args.dataset_file == 'vcoco' else test_stats['mAP']
        if args.output_dir and current_map  > best_map:
            best_map = current_map
            best_checkpoint_path = output_dir / f'checkpointbest.pth'
            utils.save_on_master({
                'model': model_without_ddp.state_dict(),
                'optimizer': optimizer.state_dict(),
                'lr_scheduler': lr_scheduler.state_dict(),
            ...
            if args.hoi and utils.is_main_process():
                with open(output_dir / "best_predictions.json", "w") as f_:
                    json.dump(preds, f_)
